...
public class LineGraphManager : MonoBehaviour {
	...
	void Start(){
       ...
       // System.IO.File.AppendAllText(Application.dataPath + "\\LegHealth.txt", "1");
        System.IO.File.AppendAllText(Application.dataPath+ "\\LegHealth.txt", ","+GM.legCoinTotal.ToString());
        string legText = System.IO.File.ReadAllText(Application.dataPath+ "\\LegHealth.txt");
        string[] legSplittedText = legText.Split(',');
        for (int i=1;i<legSplittedText.Length;i++)
        {
           // Debug.Log(splittedText[i]);
            GraphData gd = new GraphData();
            gd.marbles = float.Parse(legSplittedText[i]);
            graphDataPlayer1.Add(gd);
        }

       // System.IO.File.AppendAllText(Application.dataPath + "\\HandHealth.txt", "1");
        System.IO.File.AppendAllText(Application.dataPath + "\\HandHealth.txt",","+GM.handCoinTotal.ToString());
        string handText = System.IO.File.ReadAllText(Application.dataPath + "\\HandHealth.txt");
        string[] handSplittedText = handText.Split(',');
        for (int i = 1; i < handSplittedText.Length; i++)
        {
            // Debug.Log(splittedText[i]);
            GraphData gd = new GraphData();
            gd.marbles = float.Parse(handSplittedText[i]);
